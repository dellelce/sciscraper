...
class Scrapers(Enum):
    ...
    SUMMARY = DocScraper(config.bycatch_words,
                         config.target_words,
                         is_pdf=False)
    DIMENSIONS = WebScraper(config.citations_dataset_url,
                            api_keys=DIMENSIONS_AI_KEYS)
    SEMANTIC_SCHOLAR = WebScraper(config.citations_dataset_url,
                                  api_keys=SEMANTIC_SCHOLAR_KEYS)


class SciScraper_Prefabs(Enum):
    SCRAPE_DIRECTORY = SciScraper(config,
                                  serializer=SERIALIZERS["DIRECTORY"],
                                  scraper=Scrapers.PDF.value)
    SCRAPE_DIMENSIONS_AI = SciScraper(config,
                                      serializer=SERIALIZERS["DOI"],
                                      scraper=Scrapers.DIMENSIONS.value)
    GET_ABSTACTS_FROM_DATAFRAME = SciScraper(
        config,
        serializer=SERIALIZERS["ABSTRACTS"],
