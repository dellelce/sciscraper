...
def filter_types(target) -> list:
    ...
    """
    return [
        search_text for search_text in target
...
def serialize_from_csv(target: str, column: str = "doi") -> list[str]:
    ...
    with open(target, newline="", encoding=UTF) as file:
        data_from_csv: pd.Series = pd.read_csv(
            ...
            usecols=[column]  # type: ignore
        ).drop_duplicates()[column]
        return filter_types(data_from_csv.to_list())


def serialize_from_series(target: pd.DataFrame,
                          column: str = "abstract") -> list[str]:
...
def serialize_from_array(
    ...
    """
    data_frame: pd.DataFrame = target.explode(
        (target_column_x, target_column_y))
    return filter_types(data_frame[target_column_x].tolist()), filter_types(
        data_frame[target_column_y].tolist())


def serialize_from_directory(target: str, suffix: str = "pdf") -> list[str]:
    return [
        path.join(target, file) for file in listdir(target)
